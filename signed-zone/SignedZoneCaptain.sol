...
import {SignedZoneCaptainInterface} from "./interfaces/SignedZoneCaptainInterface.sol";

import {SignedZoneControllerInterface} from "./interfaces/SignedZoneControllerInterface.sol";

import {SignedZoneCaptainEventsAndErrors} from "./interfaces/SignedZoneCaptainEventsAndErrors.sol";
 ...
 */
abstract contract SignedZoneCaptain is Ownable, SignedZoneCaptainInterface, SignedZoneCaptainEventsAndErrors {
    ...
    constructor(address signedZoneController) {
        ...
        _SIGNED_ZONE_CONTROLLER = SignedZoneControllerInterface(signedZoneController);
    }
    ...
    function initialize(
        ...
        // Call initialize.
        _initialize(initialOwner, initialRotator, initialSanitizer, zoneName, apiEndpoint, documentationURI, zoneSalt);
    }
    ...
    function _initialize(
        ...
        SignedZoneControllerInterface(_SIGNED_ZONE_CONTROLLER).createZone(
            zoneName, apiEndpoint, documentationURI, address(this), zoneSalt
        );
    }
     ...
     */
    function updateZoneAPIEndpoint(address zone, string calldata newApiEndpoint) external override {
     ...
     */
    function updateZoneDocumentationURI(address zone, string calldata newDocumentationURI) external override {
        // Ensure caller is the owner.
        _assertCallerIsOwner();

        // Call to the signed zone controller to update the zone documentation
        // URI.
        _SIGNED_ZONE_CONTROLLER.updateDocumentationURI(zone, newDocumentationURI);
    }
     ...
     */
    function updateZoneSigner(address zone, address signer, bool active) external override {
        // Ensure caller is the owner.
        _assertCallerIsOwner();
     ...
     */
    function transferZoneOwnership(address zone, address newPotentialOwner) external override {
        // Ensure caller is the owner.
        _assertCallerIsOwner();
     ...
     */
    function rotateSigners(address zone, address signerToRemove, address signerToAdd) external override {
    ...
    function sanitizeSignedZone(address zone) external override {
        ...
        address[] memory signers = _SIGNED_ZONE_CONTROLLER.getActiveSigners(zone);
    ...
    function _assertCallerIsOwner() internal view virtual {
        // Ensure caller is the owner.
        if (msg.sender != owner()) {
