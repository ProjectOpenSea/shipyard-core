 ...
 */
contract SignedZone is SignedZoneEventsAndErrors, ZoneInterface, ISIP5 {
        ...
        returns (bytes4 validOrderMagicValue)
    {
        ...
        assembly {
            ...
            let subStandardVersionByte :=
                shr(248, calldataload(add(extraDataPtr, ExtraData_substandard_version_byte_offset)))
            ...
            let considerationLength := calldataload(add(0x24, calldataload(Zone_consideration_head_cdPtr)))
    ...
    function supportsInterface(bytes4 interfaceId) external view override(IERC165, ZoneInterface) returns (bool) {
     ...
     */
    function _supportsInterface(bytes4 interfaceId) internal pure returns (bool _supports) {
        // Determine if the interface is supported.
        _supports = interfaceId == type(ISIP5).interfaceId // SIP-5
        ...
        )
    {
        ...
        (, zoneName, apiEndpoint, substandards, documentationURI) =
            SignedZoneControllerInterface(_controller).getAdditionalZoneInformation(address(this));
    }
     ...
     */
    function _recoverSigner(bytes32 digest, bytes memory signature) internal view returns (address recoveredSigner) {
        ...
        returns (address expectedFulfiller)
    {
        ...
        assembly {
            ...
            let actualReceivedIdentifier := calldataload(add(actualReceivedIdentifier_offset, considerationPtr))

            // Get the expected received identifier.
            let expectedReceivedIdentifier := calldataload(add(expectedReceivedIdentifier_offset, extraDataPtr))
