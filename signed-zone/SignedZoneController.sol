 ...
 */
contract SignedZoneController is SignedZoneControllerInterface, SignedZoneControllerEventsAndErrors {
    /**
    ...
    function createZone(
        ...
        bytes memory _SIGNED_ZONE_CREATION_CODE = abi.encodePacked(type(SignedZone).creationCode, abi.encode(zoneName));

        // Using assembly try to deploy the zone.
        assembly {
            signedZone := create2(0, add(0x20, _SIGNED_ZONE_CREATION_CODE), mload(_SIGNED_ZONE_CREATION_CODE), salt)

            if iszero(extcodesize(signedZone)) { revert(0, 0) }
        }
     ...
     */
    function updateDocumentationURI(address zone, string calldata documentationURI) external override {
        // Ensure the caller is the owner of the signed zone.
        _assertCallerIsZoneOwner(zone);
        ...
        if (active) {
            ...
            for (uint256 i = 0; i < signedZoneProperties.activeSignerList.length;) {
                if (signedZoneProperties.activeSignerList[i] == signer) {
                    signedZoneProperties.activeSignerList[i] =
                        signedZoneProperties.activeSignerList[signedZoneProperties.activeSignerList.length - 1];
     ...
     */
    function getActiveSigners(address zone) external view override returns (address[] memory signers) {
        // Ensure that the zone in question exists.
        _assertZoneExists(zone);
     ...
     */
    function getZone(string memory zoneName, bytes32 salt) external view override returns (address derivedAddress) {
        // Get the zone creation code hash.
        bytes32 _SIGNED_ZONE_CREATION_CODE_HASH =
            keccak256(abi.encodePacked(type(SignedZone).creationCode, abi.encode(zoneName)));
        // Derive the SignedZone address from deployer, salt and creation code
        // hash.
        derivedAddress = address(
            uint160(
                uint256(keccak256(abi.encodePacked(bytes1(0xff), address(this), salt, _SIGNED_ZONE_CREATION_CODE_HASH)))
            )
        );
    }
     ...
     *                             removed.
     */
    function _assertSignerPermissions(SignedZoneProperties storage signedZoneProperties, address signer, bool active)
        private
        view
    {
