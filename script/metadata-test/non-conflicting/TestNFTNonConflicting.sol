...
/**
 * @title TestNFTNonConflicting
 * @notice Test contract with both dynamic traits AND tokenURI metadata traits.
 *         The trait keys are NON-CONFLICTING (different keys in each source).
 *
 *         tokenURI traits: "Background", "Rarity", "Generation"
 *         Dynamic traits: "Level", "Experience", "Guild"
 */
contract TestNFTNonConflicting is DynamicTraits, Ownable, ERC721 {
    uint256 public currentId;

    // Static trait data stored per token
    mapping(uint256 => string) public tokenBackground;
    mapping(uint256 => string) public tokenRarity;
    mapping(uint256 => uint256) public tokenGeneration;

    constructor() ERC721("TestNFT NonConflicting", "TNNC") {
    ...
    function _stringURI(uint256 tokenId) internal view returns (string memory) {
        return json.objectOf(
            Solarray.strings(
                json.property("name", string.concat("NonConflicting Test #", LibString.toString(tokenId))),
                json.property("description", "Test NFT with non-conflicting dynamic and tokenURI traits"),
    ...
    function _attributesArray(uint256 tokenId) internal view returns (string memory) {
        // These are the STATIC traits from tokenURI
        return json.arrayOf(
            Solarray.strings(
                Metadata.attribute("Background", tokenBackground[tokenId]),
                Metadata.attribute("Rarity", tokenRarity[tokenId]),
                Metadata.attribute("Generation", LibString.toString(tokenGeneration[tokenId]))
            )
        );
    }
    ...
    function setStaticTraits(uint256 tokenId, string calldata background, string calldata rarity, uint256 generation)
        external
        onlyOwner
    {
